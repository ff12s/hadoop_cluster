...
1. **Версия OL-Spark < 1.38.** До 1.38.0 column lineage для
   ...
   пересобрать и поднять заново: `start-cluster.bat --build --all`. Флаг `--build`
   обязателен: без него скрипт тянет образы с Docker Hub и перетирает ими только что
   собранные локальные теги, из-за чего исправление молча не подействует.
