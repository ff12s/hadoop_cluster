...
def ol_macro(field: str, forced: bool | None = None, dag_cur: str | None = None) -> str:
    ...
    :param dag_cur: DAG-значение того же ключа, переданное **парсом**. Три состояния:
      ...
      **Выключение возвращает DAG-значение, а не пустую строку.** Любой отказной выход
      (``forced=False``, ``enabled=false``, ``_cfg`` вернул None, ``_validate_cfg`` вернул
      None) отдаёт ``dag_cur`` — то есть ровно то, что DAG задал сам. Иначе при безопасном
      канале, где весь conf-ключ равен вызову макроса, выключенный лайнидж стирал бы
      DAG-овские ``spark.extraListeners`` и ``jars``, к лайниджу отношения не имеющие:
      оператор гасит Variable, а вместе с ней уезжает чужой listener и чужой jar.
      Канал ``""`` при этом даёт ``""`` (DAG молчал), канал ``None`` — тоже ``""``
      (текст DAG'а уже стоит слева).
...
| `enabled: true`, всё валидно, `openlineage_jar` валиден, **probe `False`** | **лайнидж выключен целиком**: ни jar, ни listener, ни url/namespace | warning «`openlineage_jar` не подтверждён в HDFS» |

> **Исправлено в Ревизии 3 после финального ревью.** Ревизия 2 допускала, что при отказавшем
> зонде listener всё-таки попадёт в conf, и объявляла падение джобы «тем самым отказом, против
> которого зонд». Это ровно наоборот: `spark.extraListeners` без jar'а на classpath роняет
> драйвер `ClassNotFoundException`, то есть зонд не предотвращал поломку, а вызывал её — и
> мемо удерживало это состояние 300 с. Предковая схема при недоступном jar'е не инжектила
> ничего. Инвариант 19 возвращает это поведение: ветки `listener`, `url` и `namespace` тоже
> гейтятся на успешном зонде. Мемо `jar_available` по URI делает лишний вызов бесплатным.
...
- **Инвариант 19 — нет jar'а, нет и listener'а.** Если зонд не подтвердил `openlineage_jar` в
  HDFS, лайнидж выключается **целиком**: `ol_macro` отдаёт `_refusal(dag_cur)` не только по
  ветке `jar`, но и по `listener`, `url`, `namespace`. Инжектировать listener без jar'а —
  значит уронить драйвер `ClassNotFoundException`; политика обязана деградировать в «лайниджа
  нет», а не в «джоба не стартует». Проверяется тестом «probe False → conf без OL-ключей».
- **Инвариант 18 — выключенный лайнидж не трогает чужое.** Отказной выход `ol_macro` отдаёт
  `dag_cur`, а не `""`. Значение, которое DAG задал сам (`spark.extraListeners`, `jars`,
  `conf["spark.jars"]`), обязано пережить и `enabled=false`, и отсутствующую Variable, и
  негодный конфиг. Проверяется тестом на паре «выключено + непустой `dag_cur`».
