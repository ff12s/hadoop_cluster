...
docker exec %AIRFLOW% python -c "import json; from airflow.models import Variable; import ol_policy; Variable.set('openlineage_config', json.dumps({'enabled': True, 'spark_conf': {'spark.extraListeners': 'io.openlineage.spark.agent.OpenLineageSparkListener', 'spark.openlineage.transport.type': 'http', 'spark.openlineage.transport.url': 'http://marquez:5000', 'spark.openlineage.namespace': 'smoke-ns', 'spark.openlineage.columnLineage.datasetLineageEnabled': 'true'}, 'openlineage_jar': 'hdfs://namenode:9000/opt/openlineage/openlineage-spark_2.13-1.46.0.jar'})); cfg = ol_policy.variable._validate(ol_policy.variable._cfg()); assert cfg is not None and cfg.namespace == 'smoke-ns', 'Variable edit was not picked up'; print('variable pickup OK')" || (
