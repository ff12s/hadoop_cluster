...
# Зеркало spark/scripts/pyspark_pi.py: bind-mount ./airflow/jobs в docker-compose.yml
# перекрывает файл, встроенный в образ Dockerfile'ом (COPY --from=sparkdist), поэтому
# для SparkSubmitOperator он должен физически присутствовать здесь же.
