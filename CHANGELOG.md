...
- ol_policy: зонд WebHDFS получил SPNEGO-фолбэк при 401 Unauthorized (запрашивает Kerberos-токен
  ...
  HA-пары) поднят до 10 с (было 5 с) чтобы уместить две попытки SPNEGO с таймаутом. Ошибочные исходы
  (дедлайн, исключение, standby) мемоизируются на 30 с вместо 300 с — восстановление кластера
  подхватывается быстро, а поток тасок не долбит мёртвый кластер (авторитетные исходы найденного
  или отсутствующего jar живут 300 с).
